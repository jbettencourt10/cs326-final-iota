 ...
 */
   ...
   */
  export function MiniCrypt(its = 1e5, keyL = 64, saltL = 16, digest = 'sha256') {
    this.its = its;
    this.keyL = keyL;
    this.saltL = saltL;
    this.digest = digest;
     ...
     */
    this.check = function(pw, salt, hash) {
      return crypto.timingSafeEqual(crypto.pbkdf2Sync(pw, salt, this.its, this.keyL, this.digest), Buffer.from(hash, 'hex'));
    };
  }
